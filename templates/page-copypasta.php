<?php /* Template Name: Copypasta Page */ if(have_posts()): while(have_posts()): the_post();?>
<?php
...
if($type == 'random' || $type == '' || $type == 'f-'):
	$myQuery = new WP_Query(array('post_type' => 'copypastas', 'posts_per_page' => '-1', 'nopaging' => true));
	...
	$postId = url_to_postid('/copypastas/'. $type);
...
if($api) {
	...
	<html lang="en" dir="ltr">
		...
		<body>
			<?php $myQuery = new WP_Query(array('post_type' => 'copypastas', 'posts_per_page' => '-1', 'nopaging' => true, 'orderby' => 'title', 'order' => 'ASC'));
			if($myQuery->have_posts()): ?>
			<div class="fact-types">
				<select name="" id="fact-select" onchange="updateUrl(this)">
					...
					<option value="asdf">View All Facts</option>
